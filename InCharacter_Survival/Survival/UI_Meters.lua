...
local function MakeBar(parent, label, y)
    ...
    InCharacter.UI.Theme.InkFont(row.name)
...
local function Build()
    panel = CreateFrame("Frame", "InCharacterSurvivalMeters", UIParent, "BackdropTemplate")
    ...
    InCharacter.UI.Theme.ApplyParchmentBackdrop(panel, 0.94)

    panel.title = panel:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    panel.title:SetPoint("TOPLEFT", 12, -10)
    panel.title:SetText("Condition")
    InCharacter.UI.Theme.GoldTitle(panel.title)
    ...
    ActionBtn("Rest", 158, function()
        if UnitAffectingCombat and UnitAffectingCombat("player") then
            InCharacter.UI.Theme.Toast("You cannot truly rest in combat.")
            return
        end
    ...
    panel:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("In Character — Survival")
...
function InCharacter.Survival.UI.Init()
    Build()
    InCharacter.Survival.UI.Refresh()
end

function InCharacter.Survival.UI.Refresh()
    if not panel then return end
    local state = InCharacter.Survival.GetState()
...
function InCharacter.Survival.UI.Toggle()
...
function InCharacter.Survival.UI.ShowPanel()
    if not panel then Build() end
    InCharacter.CharDB.survival.hideMeters = false
    InCharacter.Survival.UI.Refresh()
    panel:Show()
end