...
InCharacter.PvP = InCharacter.PvP or {}
InCharacter.PvP.AfterAction = {}
...
function InCharacter.PvP.AfterAction.TryReport()
    local db = EnsureDB()
    ...
    InCharacter.Chronicle.Capture.AddEntry("PVP", facts, "auto")
...
function InCharacter.PvP.AfterAction.Init()
    EnsureDB()
    ...
    C_Timer.NewTicker(5, function()
        ...
        if inInstance and (instanceType == "pvp" or instanceType == "arena") then
            ...
            local winner = GetBattlefieldWinner and GetBattlefieldWinner()
            if winner ~= nil then
                InCharacter.PvP.AfterAction.TryReport()
            end
        end
    end)
end

function InCharacter.PvP.AfterAction.SetEnabled(on)
    EnsureDB().enabled = on and true or false
end

function InCharacter.PvP.AfterAction.DebugSample()
    InCharacter.Chronicle.Capture.AddEntry("PVP", {
        outcome = "victory",
