...
function InCharacter.DetectLongevityProfile()
    local _, raceFile = UnitRace("player")
    if raceFile and InCharacter.RaceLongevityMap[raceFile] then
        return InCharacter.RaceLongevityMap[raceFile]
    end
...
function InCharacter.GetLongevityProfileId()
    local id = InCharacter.YearCalendar and InCharacter.YearCalendar.EnsureIdentity and InCharacter.YearCalendar.EnsureIdentity()
    if id and id.longevityProfile and id.longevityProfile ~= "auto" then
        if InCharacter.LongevityProfiles[id.longevityProfile] then
            return id.longevityProfile
        end
    end
    return InCharacter.DetectLongevityProfile()
end

function InCharacter.GetLongevityProfile()
    local pid = InCharacter.GetLongevityProfileId()
    return InCharacter.LongevityProfiles[pid] or InCharacter.LongevityProfiles.mortal, pid
end

function InCharacter.GetAgeBand(ageYears)
