...
function InCharacter.GetEraById(eraId)
    for _, e in ipairs(InCharacter.TimelineEras) do
        if e.id == eraId then return e end
    end
    return nil
end

function InCharacter.GetEraAtYear(yearADP)
...
function InCharacter.ErasOverlapping(startY, endY)
    local list = {}
