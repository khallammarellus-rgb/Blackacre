...
local function Cfg()
    return InCharacter.BeaconConfig or {
        ROLL_INTERVAL = 240,
        MIN_ROLL_GAP = 60,
        HEAD_HOLD = 5,
        YELL_RANGE_YARDS = 40,
...
local function ReceiveEnabled()
    local p = InCharacter.Lifecycle and InCharacter.Lifecycle.EnsurePresenceDB and InCharacter.Lifecycle.EnsurePresenceDB()
...
local function SetHoodModel()
    ...
    model:ClearModel()
    local ids = InCharacter.BeaconHoodDisplayIDs or { 10044 }
...
local function EnsureFrame()
    ...
    frame = CreateFrame("Frame", "InCharacterBeaconHead", UIParent, "BackdropTemplate")
    ...
    if InCharacter.UI and InCharacter.UI.Theme and InCharacter.UI.Theme.ApplyParchmentBackdrop then
        InCharacter.UI.Theme.ApplyParchmentBackdrop(frame, 0.92)
    else
        frame:SetBackdrop({
    ...
    if InCharacter.UI and InCharacter.UI.Theme and InCharacter.UI.Theme.InkFont then
...
function InCharacter.BeaconHead.ShowBeacon(beacon)
...
function InCharacter.BeaconHead.RollNext()
    ...
    InCharacter.BeaconHead.ShowBeacon(pick)
end

function InCharacter.BeaconHead.OnCacheChanged()
    -- soft: do not force immediate roll on every packet
end

function InCharacter.BeaconHead.Init()
    EnsureFrame()
    ...
    f:SetScript("OnEvent", function(_, event)
        ...
        if event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
            C_Timer.After(3, function()
                InCharacter.BeaconHead.RollNext()
            end)
        else
