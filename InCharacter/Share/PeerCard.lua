...
function InCharacter.Share.OnPeerSummary(sender, data)
    if not data then return end
    local honor = data.clean and "unstained" or "stained"
    local lines = {
        string.format("|cffc9a227In Character card — %s|r", data.name or sender),
    ...
    if InCharacter.UI and InCharacter.UI.Theme then
        InCharacter.UI.Theme.Toast("Received summary from " .. (data.name or sender))
    end
end