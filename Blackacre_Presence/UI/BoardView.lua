...
local function SealPrefix(scopeTier)
    if Blackacre.UI and Blackacre.UI.Theme and Blackacre.UI.Theme.FormatSealPrefix then
        return Blackacre.UI.Theme.FormatSealPrefix(scopeTier)
    end
...
function Blackacre.BoardView.Init()
    frame = CreateFrame("Frame", "BlackacreBoardView", UIParent, "BackdropTemplate")
    ...
    frame:Hide()
    tinsert(UISpecialFrames, "BlackacreBoardView")
    ...
    knownFrame = CreateFrame("Frame", "BlackacreKnownBoards", UIParent, "BackdropTemplate")
    knownFrame:SetSize(320, 340)
    knownFrame:SetPoint("CENTER")
    knownFrame:SetFrameStrata("DIALOG")
    if Blackacre.UI and Blackacre.UI.Theme and Blackacre.UI.Theme.ApplyParchmentBackdrop then
        Blackacre.UI.Theme.ApplyParchmentBackdrop(knownFrame, 0.97)
    end
    knownFrame:Hide()
    tinsert(UISpecialFrames, "BlackacreKnownBoards")
    ...
    if Blackacre.UI and Blackacre.UI.Theme then
        Blackacre.UI.Theme.GoldTitle(knownFrame.title)
    end
...
function Blackacre.BoardView.ShowKnownBoards()
    ...
    local all = Blackacre.Boards.GetAll()
    local lines = {}
    local seen = {}
...
function Blackacre.BoardView.CheckProximity()
    ...
    if board and board.id ~= (currentBoard and currentBoard.id) then
        ...
        Blackacre.Comms.BroadcastBoardQuery(board.id)
        if frame then
...
function Blackacre.BoardView.Refresh()
    if not frame or not frame:IsShown() or not currentBoard then return end
    Blackacre.BoardView.Populate(currentBoard.id)
end

function Blackacre.BoardView.Populate(boardId)
        ...
        if i <= shown then
            ...
            row:SetScript("OnClick", function()
                if bulletin.charName then
                    Blackacre.Comms.RequestBulletinFull(bulletin.charName, bulletin.id)
                end
            end)
        else
            row:Hide()
...
function Blackacre.BoardView.Show()
    Blackacre.BoardView.CheckProximity()
    if not currentBoard then
        Blackacre.Print("No bulletin board nearby. Opening known boards list.")
        Blackacre.BoardView.ShowKnownBoards()
        return
    end
...
function Blackacre.BoardView.OnBulletinDiscovered(bulletin)
    ...
    if currentBoard and bulletin.boardId == currentBoard.id then
        ...
        if not frame:IsShown() then
            Blackacre.Print("New bulletin at " .. (currentBoard.displayName or "nearby board") .. ".")
        end
    end
end

Blackacre.BoardView.OnNoticeDiscovered = Blackacre.BoardView.OnBulletinDiscovered

function Blackacre.BoardView.OnBulletinFullReceived(bulletin)
    if bulletin then
        local seal = SealPrefix(bulletin.scopeTier)
        Blackacre.Print("|cffffffff" .. seal .. (bulletin.title or "Bulletin") .. "|r")
        Blackacre.Print(bulletin.bodyText or "")
        nearbyBulletins[bulletin.id] = bulletin
        Blackacre.BoardView.Refresh()
    end
end

Blackacre.BoardView.OnNoticeFullReceived = Blackacre.BoardView.OnBulletinFullReceived
