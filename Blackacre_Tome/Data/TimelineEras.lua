...
function Blackacre.MinAdultBirthADP()
    local present = 25
    if Blackacre.YearCalendar and Blackacre.YearCalendar.GetPresentADP then
        present = Blackacre.YearCalendar.GetPresentADP() or present
    elseif Blackacre.Compat and Blackacre.Compat.DefaultPresentADP then
        present = Blackacre.Compat.DefaultPresentADP() or present
    end
    return present - 18
end

function Blackacre.ListEras()
    local src = Blackacre.TimelineEras or {}
    local forever = Blackacre.Compat and Blackacre.Compat.IsForever and Blackacre.Compat.IsForever()
    local out = {}
    for i = 1, #src do
        local e = src[i]
        if e.id == "vanilla" then
            -- Age of Adventure: too young at present
        elseif forever and e.id == "third_war" then
            -- present-18 is year 7, outside Third War
        elseif forever and e.foreverHide then
            -- expansion eras
        else
            out[#out + 1] = e
        end
...
function Blackacre.GetEraAtYear(yearADP)
    ...
    local eras = Blackacre.ListEras and Blackacre.ListEras() or Blackacre.TimelineEras
