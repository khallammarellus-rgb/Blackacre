...
function Blackacre.Voice.ListProfiles()
    ...
    for id in pairs(Blackacre.VoiceLexicon or {}) do
        list[#list + 1] = id
    end
    table.sort(list)
    return list
...
function Blackacre.Voice.Apply(text, profileId)
    if not text or text == "" then return text end
    profileId = profileId or Blackacre.Voice.ResolveProfile()
    if not profileId or profileId == "none" then return text end
    local lex = Blackacre.VoiceLexicon and Blackacre.VoiceLexicon[profileId]
